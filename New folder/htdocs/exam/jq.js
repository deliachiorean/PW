$(document).ready(function () {
    ...
    function create(data) {
        ...
        $.each(data, function (i, item) {
            trHTML += '<tr>' +
                '<td id=' + item.username + '>' + item.username + '</td>' +
                '<td id=' + item.password + '>' + item.password + '</td>' +
                '<td id=' + item.email + '>' + item.email + '</td>' +
                '<td id=' + item.adresa + '>' + item.adresa + '</td>' +
                '<td id=' + item.initiale + '>' + item.initiale + '</td>' +
                '<td id=' + item.suc + '>' + item.suc + '</td>' +
                '</tr>';
            $('#useri').append(trHTML);
        });
        $('#getUsers').hide();
    ...
    $('th').click(function () {
        ...
        $.each(rows, function (index, row) {
            $table.children('table').append(row);
        });
        return false;
    });
    ...
    //     table = document.getElementById("useri");
    //     switching = true;
